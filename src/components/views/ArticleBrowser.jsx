...
export default function Home() {
  ...
  async function loadArticlePreviews(query) {
    try {
      setArticles((await axios.get('/api/v1/articles/previews', {params: query})).data);
    }
    catch (error) {
      console.error(error)
    }
  ...
  useEffect(() => {
    ...
    return (
      ...
      <div><h2 className='font-bold text-2xl'>Search articles</h2></div>
      <div className='flex mb-8'>
        <CustomInput className="text-2xl w-full h-10 mr-1" inputClassName='h-[38px]' onChange={updateSearchQuery}/>
