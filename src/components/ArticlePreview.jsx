...
export default function ArticlePreview({article}) {
  return (
    <Link href={"/articles/read?title=" + article.title}>
        <div className='w-full hover:bg-neutral-100 dark:hover:bg-neutral-800 p-2 mb-2'>
            ...
            <div>
                <span className='mr-4 text-green-400 dark:text-green-600'><FontAwesomeIcon className='mr-1' icon={faArrowUp} />{article.upvotes}</span>
                <span className='mr-4 font-bold'>{article.upvotes - article.downvotes}</span>
                <span className=' text-red-400 dark:text-red-600'>{article.downvotes}<FontAwesomeIcon className='ml-1' icon={faArrowDown} /></span>
            </div>
            <div>
            <strong>Tags: </strong>
                <ul>
